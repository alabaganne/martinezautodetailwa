...
export const testCatalogItems = async () => {
  try {
    ...
    const response = await catalogApi.listCatalog({ types: 'ITEM'});
    ...
    return {
      ...
      data: {
        ...
        // Keep it simple for the test, just return a few items
...
export const testTeamMembers = async (locationId) => {
  try {
    ...
    const response = await fetch(`/api/team-members?location_ids=${locationId}`);
...
export const testBookingsApi = async (locationId) => {
  try {
    ...
    const response = await fetch(`/api/bookings?location_id=${locationId}&limit=10`);
    const data = await response.json();
