...
const ServiceSelection: React.FC<StepProps> = ({ formData, setFormData }) => {
	...
	const estimatedPrice = serviceType && vehicleType && vehicleCondition ? calculatePrice(vehicleType, serviceType, vehicleCondition === 'very-dirty') : 0;
					...
					{VEHICLES.map((vehicle) => {
						...
						return (
							<button
								...
								{serviceType && priceAdjustment > 0 && <div className="text-xs text-blue-600 mt-2 font-medium">+{displayPrice(priceAdjustment)}</div>}
								{isSelected && (
