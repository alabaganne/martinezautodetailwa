	...
	const fetchLocationInfo = async () => {
		try {
			...
			if (data.locations && data.locations.length > 0) {
				...
				// Determine if sandbox based on currency (USD = sandbox for Tunisia account)
				// or if the name contains "Test" or "Sandbox"
				const isTestAccount =
					location.name?.toLowerCase().includes('test') || location.name?.toLowerCase().includes('sandbox') || location.currency === 'USD';
				setIsSandboxMode(isTestAccount);
			}
		} catch (error) {
						...
						<div className="flex items-center space-x-2">
							<span
								className={`px-3 py-1 rounded-full text-sm font-medium ${
									isSandboxMode ? 'bg-yellow-100 text-yellow-800' : 'bg-green-100 text-green-800'
								}`}>
					...
					<div className="bg-amber-50 border border-amber-200 rounded-lg p-4 mb-6">
						<div className="flex items-start">
							...
							<div className="text-sm">
								...
								<p className="text-amber-700 mt-1">You're using sandbox credentials. All operations are safe and won't affect real data.</p>
							</div>
						</div>
					</div>
				)}
								...
								<div className="flex items-start justify-between">
									<div className="flex items-start space-x-4">
										...
										<div className="flex-1">
											...
											{/* Result Display */}
											{result && (
												<div className={`mt-4 p-3 rounded-lg ${result.success ? 'bg-green-50' : 'bg-red-50'}`}>
									...
									<button
										...
										className="flex items-center gap-2 px-3 py-1.5 bg-brand-100 text-brand-700 rounded-lg hover:bg-brand-200 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
										{isLoading ? (
											<>
					...
					<div className="mt-6 bg-white rounded-lg shadow-md p-6">
						...
						<div className="grid grid-cols-3 gap-4">
							<div className="text-center">
								<p className="text-2xl font-bold text-gray-800">{Object.keys(results).length}</p>
								<p className="text-sm text-gray-600">Total Tests</p>
							</div>
							<div className="text-center">
								<p className="text-2xl font-bold text-green-600">{Object.values(results).filter((r) => r.success).length}</p>
								<p className="text-sm text-gray-600">Passed</p>
							</div>
							<div className="text-center">
								<p className="text-2xl font-bold text-red-600">{Object.values(results).filter((r) => !r.success).length}</p>
